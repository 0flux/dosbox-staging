...
struct LocaleInfoEntry {
	...
	LocaleSeparator list_separator =
	    (thousands_separator != LocaleSeparator::Comma &&
	     decimal_separator != LocaleSeparator::Comma)
	        ? LocaleSeparator::Comma : LocaleSeparator::Semicolon;
};
...
enum class KeyboardScript : uint8_t {
	// Latin scripts
	LatinQwerty,
	LatinQwertz,
	LatinAzerty,
...
namespace LocaleData {
	...
	extern const std::map<DosCountry, CountryInfoEntry> CountryData;
};
...
struct HostLocale {
	...
	// these values
	std::string language              = {}; // translation (messages)
	std::string layout                = {}; // FreeDOS keyboard layout
...
// Functions for the KEYB command output
